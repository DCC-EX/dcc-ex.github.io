...
.. note:: 
  ...
  When tuning positions, you can use the ``<D TT vpin steps activity>`` diagnostic command as outlined in :ref:`ex-turntable/test-and-tune:tuning your turntable positions` to test and refine these for perfect track alignment between the turntable bridge track and the surrounding tracks. You can also use the direct EX-Turntable serial testing command ``<M steps activity>`` as covered in :ref:`ex-turntable/test-and-tune:ex-turntable interactive serial console commands`.
