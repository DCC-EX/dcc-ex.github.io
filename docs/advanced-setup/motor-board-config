...
You will see a list of supported boards with their type. Continue below.
...
If your board is not in the list, you can easily add it. In your config.h file, find the line that looks like this:
...
The PROG track requires current sense to detect the current pulse back from decoders to ACKnowledge the receipt of your command. The DCC specification says that a decoder must send a short 60mA (60 milliAmps) or more current pulse to the programming track for at least 6ms (6 milliseconds). You may still be able to have the decoder take a command, just as you would for POM (programming on main), but you will recieve no acknowledgement from the loco and you will have no way to read CVs.

In order to calculate the current, we need to know the "Volts per Amp" reported by the motor board current sense circuit. For example, the Arduino Motor Shield, using the L298 dual H-Bridge, has a special circuit that gives us 1.65V/A (1.65 Volts per Amp) reported. In theory, that means this board would sent 1.65V to our Arduino analog sense pin when 1A of current was flowing from the motor board. When 2A was flowing, we should see 3.3V on our sense pin.

The Arduino has an ADC (Analog to digital converer) that reads this analog voltage, samples it, and convers it to a digital reading. Ardunio pins have a 10bit resolution, that means it can hold a maximum value of 1024. What hat means to us is that the current is expressed as a number from 0 to 1023. Therefore, we need a formula to convert the raw Arduino pin reading to a current in milliAmps. Here is the formula we use:

.. code:: cpp

senseFactor = ((5/1024)/Volt/Amp)*1000

The Arduno analog pin can go from 0 to 5V and has 1024 possible levels, so we divide 5 by 1024, then divide by the V/A figure from the motor board current sense output, then multiply it by 1000 to make the number easier to work with. From our example of the Arduino motor shield above and it's 1.65V/A, we can compute the senseFactor as follows:
...
We use the senseFactor to calculate our current in milliAmps by just taking a raw pin reading and multiplying it by this current senseFactor. Again using the Arduino Motor Shield values, if we got a reading of 300 out of 0-1023, that would be 300 * 2.99 or about 897mA.

You will also note that if you have the maximum of 2A flowing for this board (2000mA), that the pin readin will only be around 669. That isn't very close to 1023. That is because the Arduino Motor Shield actually only reports its maximum current of 2 Amps as 3.3V, not 5. That would let you use a 3.3V microcontroller with this motor shield.

Many of the stand-alone (discrete) motor boards like the L298N or IBT_2 require a current sense resitor connected between the CS pin on the motor board and ground. This creates a voltage we can read by then connecting the CS pin to our analog pin (usually A0 or A1). This resistor needs to be very small, usually .15 to .25 ohms. We don't want a large voltage drop taking power away from our track (E = I * R so 2Amps at 1ohm would drop 2Volts!). We also don't want to have to have a huge resistor (P = I * E, so 2V drop in the resistor times 2 Amps of current is 4 Watts!). But, you asy, if the Arduino Motor Shield uses only a .15 Ohm resistor, that's only is only a voltage reading of 0 to .3 volts. That is much too low a reading for the Arduino to read! And that is why the motor shield has an op amp circuit that multiple this voltage by 11 to bring it to 3.3 Volts that an Arduino can read.
...
In some cases, the datasheet for your motor shield will list it. If the board or chip only provides an output, you are going to have to figure it out using ohms law. For a board like the IBT_2 that can handle 30 or more Amps, you are going to have to choose a useful range and design your current sense circuit to handle that range. We recommend using no more than 5 Amps on your main track. If you need more than 5 Amps, you need separate power districts and separate boosters.
