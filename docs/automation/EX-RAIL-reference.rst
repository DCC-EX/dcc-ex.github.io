...
You will note that the "value" parameter is listed as optional, but there are caveats on this:

- When an alias is used for RESERVE/FREE, LATCH/UNLATCH, or a physical or virtual pin, the value must be specified.
- If you simply require a unique ID, ``ALIAS(name)`` will calculate a unique number based on the characters in "name", which will be used in the background.
- Any ID generated by ALIAS() is guaranteed unique (providing you follow the rules below).
...
.. tip:: 
  ...
  If you want to see the ID ``ALIAS(name)`` will generate, execute ``<? name>`` in the serial console of the CommandStation. For example, to get the ID for "MY_ALIAS":

  .. code-block:: 

    <? MY_ALIAS><* Opcode=? params=1 *>
    <* p[0]=-14787 (0xFFFFC63D) *>

  The p[0] parameter is the ID, which in this case is -14787.

  *Note that while you can find this ID, don't define it with your ALIAS(), as future software updates may change this value.*
