...
- With the anode (positive) side of the lights connected to a common power source, and the cathode (negative) connected to the digital pin via a current limiting resistor - this is generally the preffered method, and is referred to as "active low" as the digital pin is set low to turn the light on
