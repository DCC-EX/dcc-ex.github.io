...
First you will need to add a new file, just like the :ref:`config.h file </ex-commandstation/get-started/arduino-ide:Copy the config.example.h file (or rename it)>`. Create a new tab using the following menu option.
...
Finally, upload the code to the Arduino as you would do during the standard :ref:`Arduino IDE Setup </ex-commandstation/get-started/arduino-ide:Upload the software>`. Restart the Command Station and these commands will have run at start up.
