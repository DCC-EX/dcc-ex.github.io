...
role="doc"} page, these instructions are primarily about using the
...
motor](/_static/images/ex-turntable/a4988-nema17-nano.png){alt="A4988 NEMA Option"}

![Connections for a DRV8825 stepper driver with NEMA17 stepper
motor](/_static/images/ex-turntable/drv8825-nema17-nano.png){alt="DRV8825 NEMA Option"}
...
`ex-turntable/configure:invert_direction`{.interpreted-text role="ref"}.

![Connections for a TMC2208 stepper driver with NEMA17 stepper
motor](/_static/images/ex-turntable/tmc2208.jpg){alt="TMC2208"}
...
> role="doc"}, and an additional folder containing the \|EX-TT\|
...
`/ex-turntable/configure`{.interpreted-text role="doc"}.
...
`/support/ex-tt-troubleshooting`{.interpreted-text role="doc"} for
...
`/ex-turntable/configure`{.interpreted-text role="doc"} page.
...
Also, if you have enabled the [FULL_STEP_COUNT]{.title-ref} option in
\"config.h\", that will prevent automatic calibration occurring, refer
to `ex-turntable/configure:full_step_count`{.interpreted-text
role="ref"}.
::::
...
- VPIN=600 - This is the default virtual pin (Vpin) ID that is used to
  ...
  `reference/developers/hal:overview`{.interpreted-text role="ref"}).
...
role="ref"} all the way through to the
`reference/developers/hal-config:upload the new version of the software`{.interpreted-text
role="ref"} step to upload your newly configured CommandStation.
