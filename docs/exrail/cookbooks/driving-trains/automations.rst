...
An :doc:`AUTOMATION </exrail/getting-started>` is a sequence of |EX-R| commands used to drive a loco.

By creating an AUTOMATION in |EX-R|, your throttle/controller app will show the automation on its **Routes** display. To send a loco along an AUTOMATION, you must first drive the loco to a point where the automation can start and then select the automation from the Routes screen of your throttle.
...
Of particular interest, compared with a PC (JMRI, iTrain, RocRail etc) running the loco, |EX-R| is only interested in the sensor your task is trying to reach. This makes it vastly more efficient than even the fastest PC system.
