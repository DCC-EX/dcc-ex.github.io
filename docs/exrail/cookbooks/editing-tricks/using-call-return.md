...
Where you want to use the same sequence of commands from several places
in other sequences you can avoid copying the repeated stuff by using
`CALL` and `RETURN`
...
- If the called sequence executes a `DONE` statement, the task is
  terminated and there is no return.
