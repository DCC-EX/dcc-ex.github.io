...
Servo turnouts/points are driven through a `PCA9685 </reference/hardware/servo-module>` servo control board or an |EX-IO|.
...
- ``pin`` = The ID of the pin the servo is connected to, which would typically be the VPin ID of the `PCA9685 </reference/hardware/servo-module>` controller board.
...
- ``description`` = A human-friendly description of the turnout that will appear in |WiThrottle| and |Engine Driver| apps. Note that this must be enclosed in quotes ``""``. In some cases the ``HIDDEN`` keyword can be used here to prevent the turnout being visible to the throttles.

An example definition for a servo connected to the second control pins of the first `PCA9685 </reference/hardware/servo-module>` connected to the CommandStation, using the slow profile for prototypical operation:
