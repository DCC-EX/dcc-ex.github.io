...
Servo turnouts/points are driven through a :doc:`PCA9685 </reference/hardware/servo-module>` servo control board or an |EX-IO|.
...
- ``pin`` = The ID of the pin the servo is connected to, which would typically be the VPin ID of the :doc:`PCA9685 </reference/hardware/servo-module>` controller board.
...
An example definition for a servo connected to the second control pins of the first :doc:`PCA9685 </reference/hardware/servo-module>` connected to the CommandStation, using the slow profile for prototypical operation:
