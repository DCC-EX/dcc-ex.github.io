...
We are full steam ahead to a v5.0 release, making sure what's in there is well tested to the limits of our resources, and has documentation available as we go. Amongst other things, this has resulted in one potentially "breaking" change for users of |EX-R|'s capability to run sequences/routes on startup.
...
As of v4.2.31, this behaviour is disabled, meaning nothing defined in "myAutomation.h" will automatically start unless preceded by the ``AUTOSTART`` directive (see :ref:`ex-rail/ex-rail-reference:ex-rail functions`).
