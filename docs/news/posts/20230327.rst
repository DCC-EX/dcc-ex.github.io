...
**Breaking change:** The implicit AUTOSTART at the beginning of the "myAutomation.h" file for |EX-R| has been disabled, as we have found it confuses a great many first time users.
  ...
  As of v4.2.31, this behaviour is disabled, meaning nothing defined in "myAutomation.h" will automatically start unless preceded by the ``AUTOSTART`` directive (see :ref:`ex-rail/ex-rail-command-reference:script/sequence types and control`).
