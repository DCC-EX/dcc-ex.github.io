...
associate the pin with a |EX-CS| 'Sensor' object (through the `<S ...>` commands), then it will
...
the input pin is switched to LOW level and a message `<Q 165>` is generated by |EX-CS| to indicate that the
